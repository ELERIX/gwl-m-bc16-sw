...
def error_message(value):
    ...
    if value != 2:
        if value != 4:
            ...
            message["Subject"] = subj + " Daily log " + es_date
    ...
    if value == 0:
        ...
        mess += ".\nPeak current:\n    Discharge    " + str(daily_max_discharge) + "A\n    Charge       " + str(daily_max_charge) + "A\nPeak SOC:\n    MAX: " + str(daily_max_soc) + "%\n    MIN: " + str(daily_min_soc) + "%\nPeak voltage:\n    MAX: " + str(daily_max_voltage) + "V\n    MIN: " + str(daily_min_voltage) + "V\n"
    elif value == 5:
...
for line in lines:
    if "Period" in line:
        ...
        try:
            ...
            print("Period is not correctly written in config file, using default value 60 seconds")
            period = 60
    if "SMTP_IP" in line:
        SMTP_IP = line[8:]
...
while 1:
    for all_files in range(0, len(comm)):
        ...
        if TBV > daily_max_voltage:
            daily_max_voltage = TBV
        if TBV < daily_min_voltage or daily_min_voltage == 0:
            daily_min_voltage = TBV
        if SOC > daily_max_soc:
            daily_max_soc = SOC
        if SOC < daily_min_soc or daily_min_soc == 0:
            daily_min_soc = SOC
        ...
        if(file_name != date):
            ...
            daily_max_voltage = 0.
            daily_min_voltage = 0.
            logwrite("Beginning new files", 0)
            for i in range(0, len(comm)):
                csv_file[i].close()
                file_name = date
                date += "_"
